 ...
 * @author Tosaka
 */
public class CustomerChangeShipControl extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet CustomerChangeShipControl</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet CustomerChangeShipControl at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            if (a == null) {
                ...
                session.setAttribute("getshippingID", shippingID);
                if (action.equals("add")) {
                    response.sendRedirect("NewShippingAddress.jsp");
                }
                if (action.equals("edit")) {
                    ShippingAddress sa = dao.getShippingAddressByShippingId(Integer.parseInt(shippingID));
                    request.setAttribute("sa", sa);
                    request.getRequestDispatcher("ChangeShip.jsp").forward(request, response);
                }
                if (action.equals("delete")) {
                    dao.deleteShippingAddress(Integer.parseInt(shippingID));
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            HttpSession session = request.getSession();
            Account a = (Account) session.getAttribute("acc");
            String getshippingID = (String) session.getAttribute("getshippingID");
            if (a == null) {
                response.sendRedirect("login.jsp");
            } else {

                int accountID = a.getuID();
                ShippingAddress sa = dao.getShippingAddressByShippingId(Integer.parseInt(getshippingID));
                ...
                if (!err.isEmpty() || !err2.isEmpty() || !err3.isEmpty() || !err4.isEmpty() || !err5.isEmpty()) {
                    ...
                    if (sa.getName().equals(fullname) && sa.getAddress().equals(address) && sa.getPhonenumber().equals(phonenum) && sa.getEmail().equals(email)) {
                        session.removeAttribute("getshippingID");
                        response.sendRedirect("shippingaddress");
                    } else {
                        session.removeAttribute("getshippingID");
                        dao.updateShippingAddress(fullname, email, address, phonenum, Integer.parseInt(getshippingID));
                        response.sendRedirect("shippingaddress");
                    }
                }
